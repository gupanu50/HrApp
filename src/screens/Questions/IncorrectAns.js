...
export default function IncorrectAns({navigation, route}) {
  ...
  const [isOpenRefModal, OpenRefModal] = useState(false);
  ...
  const NextButton = () => {
    if (activeSlideIndex + 1 == isResult.length) {
      // navigation.navigate('Home', {qid: QuizIdnew});
      if (isTips?.id) {
        navigation.navigate('Tips', {qid: isTips});
      } else {
      ...
      <>
        ...
        {item.option_type === 'text' ? (
          <>
            ...
            <TouchableOpacity
              onPress={() => {
                OpenRefModal(true), setPopupData(item);
              }}
    ...
    <SafeAreaView style={{backgroundColor: Colors.default, flex: 1}}>
      ...
      <View style={customStyle.mainContain}>
        <ScrollView showsVerticalScrollIndicator={false} style={{flex: 1}}>
          ...
          />
          {/* <CustomText style={customStyle.firsthead}>
          ...
          {question.map(item => (
            <View
              ...
              <CustomText
                style={[customStyle.questiontext, {color: item.textcolor}]}>
                {item.question}
              </CustomText>
            </View>
          ))}
          <CustomText style={customStyle.Secondhead}>Correct answer</CustomText>
          <View style={customStyle.correctans}>
            <CustomText style={customStyle.rightans}>
          ...
          <Popover
            ...
            onRequestClose={() => setShowPopover(false)}>
            <CustomText style={customStyle.Splash}>
              Loreum epsum sit dolor emit, Loreum epsum sit dolor emit, eum
              epsum sit dolor emit, Loreum epsum sit dolor emit, Loreum epsum
              sit dolor emit, Loreum epsum sit dolor emit, .
            </CustomText>
          </Popover> */}
        </ScrollView>
      ...
      <Modal
        testID={'modal'}
        isVisible={isOpenRefModal}
        onSwipeComplete={() => OpenRefModal(false)}
        onBackdropPress={() => OpenRefModal(false)}
        ...
        <View style={{padding: 20, backgroundColor: '#FFF'}}>
          <View
            ...
            <TouchableOpacity onPress={() => OpenRefModal(false)}>
              <Close height={40} width={40} />
            </TouchableOpacity>
          </View>
          {PopupData?.referenceimage ? (
            <View style={{height: '70%', width: '100%', paddingVertical: 15}}>
              <Image
                style={{height: '100%', width: '100%'}}
                source={{uri: PopupData?.referenceimage}}
              />
            </View>
          ) : null}
